...
| 7 | Explanation | No | |
| 8 | **Topic ID** | Yes | Numeric ID **or topic code** of the topic. Tried as a number first; if non-numeric, looked up as a topic code within your institute. An unrecognised value fails the row. |
| 9 | Text Format | No | `PLAIN` or `LATEX` (case-insensitive). Empty defaults to `PLAIN`. Use `LATEX` when the question or its options contain LaTeX math notation (e.g. `$\frac{1}{2}mv^2$`). |
| 10 | Option1 Text | Yes | |
| 11 | Option1 Image URL | No | |
| 12 | Option1 IsCorrect | Yes | `true` / `1` / `yes` = correct (case-insensitive); anything else = not correct. |
| 13 | Option2 Text | Yes | |
| 14 | Option2 Image URL | No | |
| 15 | Option2 IsCorrect | Yes | |
| 16+ | Option3 … (Text, Image URL, IsCorrect) | No | Options continue in repeating **triples**. |
| 19+ | Option4 … (Text, Image URL, IsCorrect) | No | |
...
Options are parsed as repeating `(Text, Image URL, IsCorrect)` triples starting at column 10, so a
...
- Topic ID is neither a valid numeric id nor a recognised topic code for the institute.
- Text Format is present but not one of `PLAIN` or `LATEX`.
...
```csv
Question Id,Question Text,Question Image URL,Difficulty Level,Question Type,Marks,Negative Marks,Explanation,Topic ID,Text Format,Option1 Text,Option1 Image URL,Option1 IsCorrect,Option2 Text,Option2 Image URL,Option2 IsCorrect,Option3 Text,Option3 Image URL,Option3 IsCorrect,Option4 Text,Option4 Image URL,Option4 IsCorrect
"Q01","What is the SI unit of force?","","EASY","MCQ","1","0","Force is measured in newtons (N).","1","PLAIN","Newton","","true","Joule","","false","Pascal","","false","Watt","","false"
"Q02","A body moves with uniform acceleration. Identify the correct v-t graph.","","MEDIUM","MCQ","4","1","Uniform acceleration gives a straight sloped line.","1","PLAIN","Graph A","","true","Graph B","","false","Graph C","","false","Graph D","","false"
"Q03","The roots of $ax^2 + bx + c = 0$ are given by:","","MEDIUM","MCQ","2","0","Quadratic formula derived by completing the square.","1","LATEX","$x = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}$","","true","$x = \frac{b \pm \sqrt{b^2 + 4ac}}{2a}$","","false","$x = \frac{-b}{2a}$","","false","$x = \frac{b}{2a}$","","false"
```
